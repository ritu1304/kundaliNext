...
import FestivalSearch from '@/components/FestivalComponent/FestivalSearch';
...
function Festival(props) {
    ...
    const [searchedFestival, setSearchedFestival] = useState([]);
    ...
    const fetchFestival = async (param) => {
        let LocalStore = localStorage.getItem('lng');
        let url1;
        if (LocalStore === "hi") {
            url1 = `http://172.31.11.181:5001/api/get_hindi_festival?month=${param.month}&year=${param.year}`;
        } else {
            url1 = `http://172.31.11.181:5001/api/get_festival?month=9&year=2024`;
        }
        try {
            ...
            console.log("======festivallljshdfjshshfdsjfhjs=====", res.data.data);
        } catch (error) {
    ...
    const fetchSearchedFestival = async (param) => {
        try {
            
            const res = await axios.get(`http://172.31.11.181:5001/api/searchUniqueFestival?festival=${param.festival}&year=${param.year}`);
            setSearchedFestival(res.data.data);
            console.log("====searched fes====", res.data.data)
        } catch (error) {
            console.error("Error searching festival:", error);
        }
    };
    var data = Array.from(searchedFestival);
    console.log("============searched festivalssss=======", data);

    const fetchFestivalCategory = async (param) => {
        let LocalStore = localStorage.getItem('lng');
        let url1;
        if (LocalStore === "hi") {
            url1 = `http://172.31.11.181:5001/api/getHindiFestivalsOnCategory?category=${param.category}&year=${param.year}`;
        } else {
            url1 = `http://172.31.11.181:5001/api/getFestivalsOnCategory?category=${param.category}&year=${param.year}`;
        }
        try {
            const res = await axios.get(url1);
            setFestivalCategory(res.data.data);
            console.log("=====cate fes====", res.data.data)
        } catch (error) {
            console.error("Error fetching festival category:", error);
        }
    };

    useEffect(() => {
        const mon = moment(currentDate).format('M');
        const yea = moment(currentDate).format('yy');
        fetchFestival({ month: mon, year: yea });
    }, []);

    useEffect(() => {
        fetchSearchedFestival({ festival: "Holi", year: "2024" });
    }, []);

    useEffect(() => {
        fetchFestivalCategory({ year: moment(currentDate).format('yy'), category: "Main" });
    }, []);
    

    const onSubmit1 = () => {
        if (festival && year) {
            fetchSearchedFestival({ festival, year });
            handleOpen();
        }
    };
    ...
    const onSubmitCategory = (e, value) => {
        e.preventDefault();
        fetchFestivalCategory({ year: categoryYear, category: value.value });
        setValuen(value);
    ...
    const submitCategoryYear = (e, value) => {
        e.preventDefault();
        fetchFestivalCategory({ year: categoryYear, category: valuen.value });
    ...
    const getFestivalBlogId = (name) => {
        ...
        let url1;
        if (LocalStore === "hi") {
            url1 = `http://172.31.11.181:5001/article/get_blog_by_festivalName?festivalName=${name}&festivalStatus=false&articleType=PUBLISH&status=true`;
        } else {
            url1 = `http://172.31.11.181:5001/article/get_blog_by_festivalName?festivalName=${name}&festivalStatus=true&articleType=PUBLISH&status=true`;
        }
        axios.get(url1)
            .then((res) => {
                    ...
                    <div className="container">
                        ...
                        <div className="row">
                            <div className="col-sm-12 col-md-12 col-lg-6 col-xl-6 ">
                                <div className="row ">
                                    ...
                                    <div className="col-sm-12 col-md-6 col-lg-6 col-xl-6 mx-2">
                                        <FestivalSearch parentCallback={callbackFunction} />

                                        {/* {LocalStore === "hi" ? <h6 className='HindiSearchNotAva mt-1'>{t('HindiSearchNotAva')}</h6> : " "} */}
                                    </div>
                                    <div className="col-sm-12 col-md-3 col-lg-2 col-xl-4 mx-2">
                                        <select name="year" id="year" className="dropdown-toggle calSearchYear form-control" onChange={(e) => setYear(e.target.value)}>
                                    ...
                                    <div className="col-sm-12 col-md-3 col-lg-4 col-xl-6 mx-4">
                                        <button type="button" onClick={(e) => onSubmit1(e)} className="btn calSearchButton">{t('SubmitFes')}</button>
                                    </div>
                                    </div>
                        ...
                        <div className="container calenderFestival  ">
                            ...
                            <div className="row calenderBox ">
                                ...
                                <center>
                                    <div className='flex-festivals'>
                                    <div className="col-sm-12 col-md-12  calHeader">
                                        <div className="row mt-4">
                                            ...
                                            <div className="  ">
                                                <button type="button" onClick={(e) => onSubmit(e)} className="btn calSearched">{t('Submit')}</button>
                                            </div>
                                        </div>
                                    </div>
                                    </div>
                                        ...
                                        <div className='col-md-4'>
                                            <table>
                                                ...
                                                <tbody>
                                                    ...
                                                    {fes && Array.isArray(fes) && fes.length > 0 && (
                                                        fes.slice(0, fes?.length / 3).map((festival, index) => (
                                                            <tr key={index}>
                                                                <td className='festival_for_category_margin_date'>{moment(festival.date).format('DD-MM-YYYY')}</td>
                                                                <td className='festival_for_category_margin_festival festivalNameHover' onClick={() => getFestivalBlogId(festival.festival)}>
                                                                    {festival.festival}
                                                                </td>
                                                            </tr>
                                                        ))
                                        ...
                                        <div className='col-md-4'>
                                            <table>
                                                ...
                                                <tbody>
                                                    ...
                                                    {fes && Array.isArray(fes) && fes.length > 0 && (
                                                        fes.slice(fes.length / 3, (fes.length / 3) * 2).map((festival, index) => (
                                                            <tr key={index}>
                                                                <td className='festival_for_category_margin_date'>{moment(festival.date).format('DD-MM-YYYY')}</td>
                                                                <td className='festival_for_category_margin_festival festivalNameHover' onClick={() => getFestivalBlogId(festival.festival)}>
                                                                    {festival.festival}
                                        ...
                                        <div className='col-md-4'>
                                            <table>
                                                ...
                                                <tbody>
                                                    ...
                                                    {fes && Array.isArray(fes) && fes.length > 0 && (
                                                            fes.slice(fes?.length / 3 * 2).map((festival, index) => (
                                                                <tr key={index}>
                                                                    <td className='festival_for_category_margin_date'>{moment(festival.date).format('DD-MM-YYYY')}</td>
                                                                    <td className='festival_for_category_margin_festival festivalNameHover' onClick={() => getFestivalBlogId(festival.festival)}>
                                                                        {festival.festival}
                            ...
                            <div className="row homeFestivalSmall mt-5" >
                                <center>
                                    <div className="scrollableFestivalList">
                                        ...
                                        {Array.isArray(fes) && fes.length > 0 ? (
                                                fes.map((festival, index) => (
                                                    <div className="row mx-2" key={index}>
                                                        <div className="col-sm-12 col-md-12 col-lg-12 col-xl-5">
                                                            {moment(festival.date).format('DD-MM-YYYY')}
                                                        </div>
                    ...
                    <div className="container">
                        <div className="row FestivalCardsCrousel">
                            ...
                            >
                                {festivalByCategory.map((item) => {
                                    return (
                                        <div className="row" onClick={e => onSubmitCategory(e, item)}>
                                            
                                            <div className="image-container zoom fesCardImage CarouselCategoryImages cursor-pointer">
                                                <img
                                                    src={item.image}
                                                    alt="category"
                                                    className="responsive-image"
