...
const Contact = () => {
    ...
    const ContactUser = async (data) => {
        ...
        try {
            const res = await axios('/api/add_Contacts', options);
            if (res.status === 200) {
            ...
            toast.error(error?.response?.data?.status?.message || "An error occurred");
        }
    };
    ...
    const submit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            await ContactUser(form);
            setIsSubmitting(false);
        }
    };

    return (
        <div className='container-fluid contactUsBg wrapper1 mt-10'>
            <ToastContainer />
                            ...
                            <div className="row">
                                ...
                                <div className='col-sm-12 col-md-12 col-lg-6 mt-4 border'>
                                    <form className='mb-3' onSubmit={submit}>
                                        ...
                                        <textarea className='form-control contect_us_form_control' id="msg" placeholder='Message' name="msg" value={msg} onChange={(e) => setMsg(e.target.value)} rows="2" cols="20">
                                        </textarea>
                                        {errors.msg && (<p className='errors'>{errors.msg}</p>)}
                                        <center>
                                            <button className='btn contect_us_submit_btn' type='submit'>Send</button>
                                        </center>
                                    </form>
                                </div>
                            </div>
